...
const TimetableModel = {
    getByTeacherId: function (teacherId, callback) {
        ...
        WHERE teacher_id = ? 
        ORDER BY tt.day_of_week ASC, start_time ASC
        `;
        db.query(sql, [teacherId], callback);
    },

    checkForInsert: function (teacher_id, day_of_week, start_time, end_time, callback) {
        const sql_query = `
        ...
        AND end_time = ?
    `;
        db.query(sql_query, [teacher_id, day_of_week, start_time, end_time], callback);
    },
    ...
    checkForUpdate: function (teacher_id, day_of_week, start_time, end_time, timetableId, callback) {
        const sql_query = `
        SELECT * FROM timetable 
        WHERE teacher_id = ? 
        AND day_of_week = ? 
        ...
        AND id != ?
    `;
        db.query(sql_query, [teacher_id, day_of_week, start_time, end_time, timetableId], callback);
    },

...
const Student = {
    ...
    create: function (studentData, callback) {
        const sql_query = `
        INSERT INTO students (name, email, phone, dob, gender, city, file_path, password, role) 
        VALUES (?)
        `;
    ...
    updatewith_Pic: function (studentID, studentData, callback) {
        const sql_query = `
        UPDATE students 
        SET name=?,email=?, phone=?, city=?, file_path=? 
        WHERE id=?
        `;
    ...
    updatewithout_Pic: function (studentID, studentData, callback) {
        const sql_query = `UPDATE students SET name=?, email=?, phone=?, city=? WHERE id=?`;
        db.query(sql_query, [...studentData, studentID], callback);
    },
